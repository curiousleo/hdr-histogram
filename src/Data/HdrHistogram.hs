...
-- | A pure 'Histogram'
data Histogram config value count
  = Histogram
...
instance
  (HasConfig config, Integral value, FiniteBits value, U.Unbox count, Integral count) =>
  Monoid (Histogram config value count)
  where
...
percentile ::
  (Integral value, Integral count, U.Unbox count, Bits value) =>
  Histogram config value count ->
