...
-- | A mutable 'Histogram'
data Histogram s c value count
  = Histogram
...
record ::
  (Integral value, Integral count, FiniteBits value, U.Unbox count, PrimMonad m) =>
  Histogram (PrimState m) c value count ->
  value ->
  m ()
...
recordValues ::
  (Integral value, Integral count, FiniteBits value, U.Unbox count, PrimMonad m) =>
  Histogram (PrimState m) config value count ->
  value ->
  count ->
  m ()
